"""Seed mock catalog data — used on first startup only when DB is empty."""
...
SEED_SUBGROUPS = [
    ...
    {"id": "wings", "category_id": "food", "name": "Chicken Wings", "sort_order": 2},
...
# Each product has subgroup_id (and inferred category_id)
SEED_PRODUCTS_RAW = {
    "beer": [
...
def expanded_seed_products():
    """Return list of products with category_id+subgroup_id resolved."""
    sub_to_cat = {s["id"]: s["category_id"] for s in SEED_SUBGROUPS}
    out = []
    for sub_id, items in SEED_PRODUCTS_RAW.items():
